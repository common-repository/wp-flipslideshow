...
//add replace on fileObj.filePath.replace("/wordpress/wp-admin/","") to workaround on windows server
	...
	test=file;
	var params = 'myajax=1&filename='+ flp_upload_dir+"/"+file.name + '&task=flp_resize_image_and_add&album_id='+jQuery('#multi_album_id').val();
...
function on_queue_completed(file)
{
	...
	flp_refresh_images_table(flp_current_album_id);
...
function flp_refresh_albums_table()
{
	...
	//sleep(2000);
	jQuery.post('index.php', 'task=flp_get_albums_table', function(res)
	{
		flp_set_data_tables();
		if (res.status == 'ok') 
		{
...
function flp_refresh_images_table(album_id)
{
	...
	jQuery.post('index.php', 'task=flp_get_albums_images_table&album_id='+album_id, function(res)
	{
		flp_set_data_tables();
		if (res.status == 'ok') 
		{
...
function re_order()
{
	...
	if( task == 'flp_reorder_album' )
		jQuery('#albums_table tbody').html('<tr><td colspan="7" align="center"><img src="'+loading.src+'" alt="" /><br/>Please wait, refreshing albums</td></tr>');
	if( task == 'flp_reorder_image' )
	...
	jQuery.get('index.php?' + params, function(res)
	{
		if( task == 'flp_reorder_image' )
		{
			flp_refresh_images_table(album_id);
		}
		if( task == 'flp_reorder_album' )
			flp_refresh_albums_table();
...
var tpager = null;
var ipager = null;
function flp_set_data_tables()
{
	//add event to bulk actions
	jQuery('.bulk_form').live('submit', flp_submit_bulk);
...
function flp_submit_bulk()
{
	var task = this.task.value;
	var album_id = null;
	if( task == 'flp_bulk_delete_albums' || task == 'flp_bulk_enable_albums' || task == 'flp_bulk_disable_albums')
	{
	...
	else if( task == 'flp_bulk_delete_images' || task == 'flp_bulk_enable_images' || task == 'flp_bulk_disable_images' )
	{
...
{
	//preload images
	loading.src = flp_url + '/images/loading.gif';
...
/*
		var ops = {
					'uploader'		: flp_url + '/js/uploadify-2.1.4/uploadify.swf',
					'script'		: flp_url + '/js/uploadify-2.1.4/uploadify.php',
					'cancelImg' 	: flp_url + '/js/uploadify-2.1.4/cancel.png',
				    'folder'    	: flp_upload_dir,
				    'auto'			: true,
					'multi'			: true,
					'fileExt'        : '*.jpg;*.gif;*.png',
...
*/
		var swfu;
			...
			var settings = {
				flash_url : flp_url+"/js/swfupload/js/swfupload.swf",
				upload_url: flp_url+"/js/swfupload/js/upload.php",
				post_params: {"PHPSESSID" : "<?php echo session_id(); ?>","folder":flp_upload_dir, "secw":flp_secw, nopar: "n" + new Date().getTime()},
				...
				// Button settings
				button_image_url: flp_url+"/js/swfupload/js/TestImageNoText_65x29.png",
				button_width: "130",
				button_height: "29",
	...
	jQuery('.flp_delete_album').live('click', flp_delete_album);
	...
	jQuery('table#album_images .enable, table#albums_table .enable').live('click', flp_enable);
	jQuery('table#album_images .disable, table#albums_table .disable').live('click', flp_disable);
