...
<div class="wrap">
	...
	<form id="add_new_album_form" action="" method="post" enctype="multipart/form-data">
		<input type="hidden" name="task" value="flp_add_new_album" />
		<table>
		<tr>
	...
	<div class="dataTable_wrapper">
		<div class="fg-toolbar ui-toolbar ui-widget-header ui-corner-tl ui-corner-tr ui-helper-clearfix">
			...
			<?php flp_get_table_actions(array('flp_bulk_delete_albums' => 'Delete', 
											'flp_bulk_disable_albums' => 'Disable', 
											'flp_bulk_enable_albums' => 'Enable')); ?>
		</div>
		...
		<?php else: ?>
		<?php require_once FLP_PLUGIN_DIR . '/html/albums_rows.php'; ?>
		<?php endif; ?>
		</tbody>
		</table>
	...
	if( isset($_REQUEST['view']) && isset($_REQUEST['album_id']))
	{
		...
		$album = $gflp->flp_get_album($album_id);
		//print_r($album);
		$images = $gflp->flp_get_album_images($album_id);
		//print_r($images); 
		...
		if (!(strstr($_REQUEST['view'], '/')) && !(strstr($_REQUEST['view'], "\\"))) {
			require_once FLP_PLUGIN_DIR . '/html/' . $_REQUEST['view'] . '.php';
		}
	} 
	?>
</div>